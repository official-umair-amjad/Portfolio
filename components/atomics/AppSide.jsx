        ...
        {USER.contact.contents
          ...
          .map((item) => (
            <li key={item.id} className="w-max">
              ...
              >
                <i
                  className={`${item.icon} text-2xl transition duration-300 hover:text-primary transform hover:-translate-y-1`}
      ...
      {[USER.contact.contents.find((item) => item.category === 'Email')].map(
        (item) => (
          <a
            ...
            className="transition duration-300 text-white tracking-widest bg-primary rounded py-4"
